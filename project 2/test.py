        ...
        if 'data' in js_data:
            for vegetable in js_data['data']:
                dict_veg = {
                    'date': date_,
                    'state': 'New Delhi',
                    'name': vegetable.get('vegetablename', ''),
                    'wholesalePrice': vegetable.get('price', ''),
                    'retailPrice': vegetable.get('retailprice', ''),
                    'shoppingmallPrice': vegetable.get('shopingmallprice', ''),
                    'units': vegetable.get('units', ''),
                    'image': vegetable['table'].get('table_image_url', '') if 'table' in vegetable else ''
                }
                data.append(dict_veg)
        else:
            print(f"No data found for {date_}.")
